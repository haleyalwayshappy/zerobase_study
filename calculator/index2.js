...
const result = document.querySelector('.result'); // calculator__result 엘리먼트와, 그 자식 엘리먼트의 정보를 모두 담고 있음.
    ...
    if (target.matches('button')) {
        if (action === 'number') {
            ...
            // ex) 15를 누르기 위해 1을 누른 상태의 분기(두자리 연속 누르기 위한 코드)
                ...
                if (previousKey === operatorForAdvanced) {
                    result.textContent = buttonContent;
                    previousKey = result.textContent;
        ...
        if (action === 'calculate') {
            ...
            if (firstNum !== '' && operatorForAdvanced === '') {
                ...
                result.textContent = calculate(result.textContent, operatorForAdvanced, result.textContent);
            } // 기존에 작성했던 calculate 함수를 이용하여 계산 상황에 따른 전달인자와 함께 호출
            else if (previousKey === result.textContent) {
                result.textContent = calculate(firstNum, operatorForAdvanced, previousNum);
                //   if(operatorForAdvanced === '+') {
                //   result.textContent = String(Number(firstNum) + Number(previousNum));
                ...
                result.textContent = calculate(result.textContent, operatorForAdvanced, previousNum);
            } else if (previousKey !== result.textContent && previousNum === '') {
                result.textContent = firstNum;
            }
        }
